...
| `extensions` | list of strings | `[".py", ".js", ".jsx", ".ts", ".tsx", ".go", ".cs"]` | File extensions to consider. Files with any other extension are ignored. |
...
| Key | Type | Default | Description |
|---|---|---|---|
| `include_private` | bool | `false` | Include symbols whose names begin with an underscore (Python) or are not exported (JS/TS/Go/C#). |
...
| `languages` | list of strings | `["python", "typescript", "go", "csharp"]` | Parsers to activate. Supported values: `python`, `typescript`, `go`, `csharp`. |
