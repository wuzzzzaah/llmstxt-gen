...
| `extensions` | list of strings | `[".py", ".js", ".jsx", ".ts", ".tsx", ".go", ".rs"]` | File extensions to consider. Files with any other extension are ignored. |
...
| Key | Type | Default | Description |
|---|---|---|---|
| `include_private` | bool | `false` | Include symbols whose names begin with an underscore (Python) or are not exported (JS/TS/Go/Rust). |
...
| `languages` | list of strings | `["python", "typescript", "go", "rust"]` | Parsers to activate. Supported values: `python`, `typescript`, `go`, `rust`. |
